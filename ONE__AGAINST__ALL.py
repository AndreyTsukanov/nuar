def mod_1():
                ...
                if abs(game.player_cords[game.player_number - 1][0] - i) > 1 or abs(game.player_cords[game.player_number - 1][1] - j) > 1 or abs(game.player_cords[game.player_number - 1][0] - i) + abs(game.player_cords[game.player_number - 1][1] - j) == 0:
                    ...
                    if game.killed_person != 0:
                        if game.people_alive >= players:
                            ...
                            game.dead_topic = tk.Label(text="Произошло Убийство!",font=('Arial, 30'),bg=game.bg_,fg='black')
                            ...
                            game.dead_button= tk.Button(text="ОК", width=7,height=2,font=('Arial, 15'),command=lambda: help_with_spawn())
    ...
    def help_but_click(x, y):
        if [x, y] not in game.Clicked_Buttons:
            if game.killed_person == 0:
                but = tk.Button(text=global_class_file_.help_to_show(game.Table[x][y][0]), bd=5,width=10, height=4, font=('Arial', 13), bg='gray', fg='black', command=lambda i=x, j=y:help_but_click(i, j))
            else:
    ...
    def help_for_begin():
        if game.num <= players:
            if len(game.Clicked_Buttons) == 1 and game.Clicked_Buttons[0] not in game.player_cords:
                ...
                game.Table[game.Clicked_Buttons[0][0]][game.Clicked_Buttons[0][1]][1] = game.num
                game.num += 1
                if game.num <= players:
                    game.hint_label = tk.Label(text="Игрок номер " + str(game.num) + ", укажите \nсвоего персонажа так, чтобы этого \nникто не видел, и нажмте \nкнопку 'Продолжить'",font=('Arial, 20'),bg=game.bg_)

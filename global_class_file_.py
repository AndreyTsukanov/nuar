...
class Game_without_cards:
    def __init__(self):
        ...
        self.niger = [i for i in range(1, players + 1)]
    ...
    for i in range(len(game.Table)):
        ...
        for j in range(len(game.Table[0])):
            if game.Table[i][j][2] == "black":
                but = tk.Button(text=help_to_show(game.Table[i][j][0]), bd=5, font=('Arial', 13), bg = game.Table[i][j][2], fg='white',width=10,height=4, command=lambda x=i, y=j:help_but_click(game, x, y))
            else:
                but = tk.Button(text=help_to_show(game.Table[i][j][0]), bd=5, font=('Arial', 13), bg = game.Table[i][j][2], fg='black',width=10,height=4, command=lambda x=i, y=j:help_but_click(game, x, y))
...
def help_for_player_turn(game):
    if game.index_turn != -1:
        for i in range(-1, game.index_turn, -1):
            game.History.pop(-1)
    game.index_turn = -1
    game.player_number = game.player_turn[(game.player_turn.index(game.player_number) + 1) % len(game.player_turn)]
...
def help_but_click(game, x, y):
    if [x, y] not in game.Clicked_Buttons:
        but = tk.Button(text=help_to_show(game.Table[x][y][0]), bd=5,width=10, height=4, font=('Arial', 13), bg='gray', fg='black', command=lambda i=x, j=y:help_but_click(game, i, j))
        ...
        if game.Table[x][y][2] == 'black':
            but = tk.Button(text=help_to_show(game.Table[x][y][0]), bd=5,width=10,height=4, font=('Arial', 13), bg=game.Table[x][y][2], fg='white', command=lambda i=x, j=y:help_but_click(game, i, j))
        else:
            but = tk.Button(text=help_to_show(game.Table[x][y][0]), bd=5,width=10,height=4, font=('Arial', 13), bg=game.Table[x][y][2], fg='black', command=lambda i=x, j=y:help_but_click(game, i, j))
        game.Buttons[x][y].destroy()
        game.Buttons[x][y] = but
        but.grid(row=x + 3, column=y+game.k, stick='wens', padx=5, pady=5)
...
def horizontal_clear(game):
    game.hint_label.destroy()
    game.hint_topic.destroy()
    game.hint_label = tk.Label(text="В каждой из " + str(len(game.Table)) + " строк выберите по одной\n чёрной ячейке для горизонтальной очистки...", font=('Arial, 15'),bg=game.bg_,fg='black')
    game.hint_topic = tk.Label(text="Подсказка:",font=('Arial, 30'),bg=game.bg_)
    if game.killed_person == 0:
        if len(game.Clicked_Buttons) != len(game.Table):
            ...
            for j in range(len(game.Table)):
                if game.Table[game.Clicked_Buttons[-j-1][0]][game.Clicked_Buttons[-j-1][1]][2] == 'black':
            ...
            if len(a) != len(game.Table):
                ...
                for i in range(1, len(game.Table) + 1):
                    (game.Table[game.Clicked_Buttons[-i][0]]).pop(game.Clicked_Buttons[-i][1])
                help_for_player_turn(game)
                help_with_players_cords(game)
        game.Clicked_Buttons = []
        Show(game)
...
def vertical_clear(game):
    game.hint_label.destroy()
    game.hint_topic.destroy()
    game.hint_label = tk.Label(text="В каждом из " + str(len(game.Table[0])) + " столбцов выберите по одной \nчёрной ячейке для вертикальной очистки...", font=('Arial, 15'),bg=game.bg_,fg='black')
    game.hint_topic = tk.Label(text="Подсказка:",font=('Arial, 30'),bg=game.bg_)
    if game.killed_person == 0:
        if len(game.Clicked_Buttons) != len(game.Table[0]):
            ...
            a = set()
            for j in range(1, len(game.Table[0]) + 1):
                if game.Table[game.Clicked_Buttons[-j][0]][game.Clicked_Buttons[-j][1]][2] == 'black':
                    a.add(game.Clicked_Buttons[-j][1])
            if len(a) != len(game.Table[0]):
                game.hint_label.place(x=30,y = 590)
                game.hint_topic.place(x=120, y=510)
            else:
                P_1 = [[game.Table[i][j] for i in range(len(game.Table))] for j in range(len(game.Table[0]))]
                for i in range(1, len(game.Table[0]) + 1):
                    (P_1[game.Clicked_Buttons[-i][1]]).pop(game.Clicked_Buttons[-i][0])
                game.Table = [[P_1[i][j] for i in range(len(P_1))] for j in range(len(P_1[0]))]
                help_for_player_turn(game)
                help_with_players_cords(game)
        game.Clicked_Buttons = []
        Show(game)

def mod_2():
    ...
    def kill():
        ...
        game.hint_label = tk.Label(
            text="Укажите живого персонажа, находящегося\n рядом с Вами, которого Вы хотите убить...",
            font=("Arial, 15"),
            bg="#DAA520",
            fg="black",
        )
        if game.killed_person == 0:
            if len(game.Clicked_Buttons) != 1:
                ...
                game.Table[game.Clicked_Buttons[-1][0]][game.Clicked_Buttons[-1][1]][2]
                == "black"
            ):
                game.hint_label.place(x=30, y=590)
                game.hint_topic.place(x=120, y=510)
                ...
                if (
                    abs(game.player_cords[game.player_number - 1][0] - i) > 1
                    or abs(game.player_cords[game.player_number - 1][1] - j) > 1
                    or abs(game.player_cords[game.player_number - 1][0] - i)
                    + abs(game.player_cords[game.player_number - 1][1] - j)
                    == 0
                ):
                    game.hint_label.place(x=30, y=590)
                    game.hint_topic.place(x=120, y=510)
                    ...
                    if game.killed_person != 0:
                        if game.people_alive >= players:
                            ...
                            game.dead_topic = tk.Label(
                                text="Произошло Убийство!",
                                font=("Arial, 30"),
                                bg="#DAA520",
                                fg="black",
                            )
                            game.dead_topic.place(x=20, y=488)
                            game.dead_label = tk.Label(
                                text="Игрок номер "
                                + str(game.player_number)
                                + " убил игрока номер "
                                + str(game.killed_person)
                                + ".\nИгрок номер "
                                + str(game.killed_person)
                            ...
                            game.dead_button = tk.Button(
                                text="Узнать нового персонажа",
                                height=2,
                                font=("Arial, 15"),
                                bg="silver",
                                command=lambda: help_with_spawn(),
                            )
                            ...
                            game.player_turn.pop(
                                game.player_turn.index(game.killed_person)
                            )
                            game.dead_topic = tk.Label(
                                text="Произошло Убийство!",
                                font=("Arial, 30"),
                        ...
                        # Q_and_K()
                        game.PLAYERS_NAMES[
                            game.PLAYERS_NAMES.index(game.Table[i][j][0])
                        ] = ""
    ...
    def question():
        ...
        game.hint_topic = tk.Label(text="Подсказка:", font=("Arial, 30"), bg="#DAA520")
        game.hint_label = tk.Label(
            text="Укажите ячейку персонажа, находящуюся рядом\n с Вашим пресонажем, чтобы опросить его...",
            font=("Arial, 16"),
            bg="#DAA520",
            fg="black",
        )
                    ...
                    for k in range(len(game.Table)):
                        for y in range(len(game.Table[0])):
                            if (
                                abs(i - k) < 2
                                and abs(j - y) < 2
                                and game.Table[k][y][1] != 0
                            ):
                                s.append(str(game.Table[k][y][1]))
                    s.sort()
                    game.dead_topic = tk.Label(
                        text="Вы успешно опросили\n игроков!",
                        font=("Arial, 30"),
                        bg="#DAA520",
                    )
                    game.dead_label = tk.Label(
                        text="Рядом с персонажем "
                        + str(game.name)
                        + "\nоказались игроки: "
                        + ", ".join(s),
                        width=34,
    ...
    game.begin_button = tk.Button(
        text="Узнать персонажа",
        bd=5,
        font=("Arial", 13),
        fg="red",
        bg="silver",
    ...
    def help_but_function():
        if game.killed_person == 0:
            ...
            game.hint_topic.destroy()
            x, y = (
                game.player_cords[game.player_number - 1][0],
                game.player_cords[game.player_number - 1][1],
            )
            ...
            game.HELP_BUTTON = tk.Button(
                text="Забыли персонажа?",
                bd=5,
                font=("Arial", 10),
                width=20,
                height=2,
                bg="green",
                command=lambda: help_but_function(),
            )
            game.ok_button_ = tk.Button(
                text="ОК",
                bd=5,
                font=("Arial", 13),
                bg="silver",
                ...
                height=2,
                command=lambda: (
                    game.ok_button_.destroy(),
                    game.hint_label.destroy(),
                    game.HELP_BUTTON.place(x=1290, y=400 + 40 * (players - 4) + 60),
                    global_class_file_.Show(game),
                ),
            )
            game.ok_button_.place(x=1330, y=550 + 40 * (players - 4))
            global_class_file_.Show(game)
            game.Table[x][y][2] = "white"
    ...
    def begin():
        ...
        begin_topic.destroy()
        game.turn = tk.Label(
            text="Ход игрока " + str(game.player_number),
            font=("Arial, 30"),
            bg="#DAA520",
        )
        ...
        Q = tk.Button(
            ...
            width=20,
            height=2,
            command=lambda: question(),
        )
        V = tk.Button(
            text="Вертикальная Чистка",
            bd=5,
            font=("Arial", 10),
            width=20,
            height=2,
            fg="red",
            bg="silver",
            command=lambda: global_class_file_.vertical_clear(game),
        )
        H = tk.Button(
            text="Горизонтальная Чистка",
            bd=5,
            font=("Arial", 10),
            width=20,
            height=2,
            fg="red",
            bg="silver",
            command=lambda: global_class_file_.horizontal_clear(game),
        )
        D = tk.Button(
            text="Вниз",
            bd=5,
            ...
            fg="red",
            bg="silver",
            command=lambda: global_class_file_.down(game),
        )
        U = tk.Button(
            text="Вверх",
            bd=5,
            ...
            fg="red",
            bg="silver",
            command=lambda: global_class_file_.up(game),
        )
        R = tk.Button(
            text="Вправо",
            bd=5,
            ...
            fg="red",
            bg="silver",
            command=lambda: global_class_file_.right(game),
        )
        L_ = tk.Button(
            text="Влево",
            bd=5,
            ...
            fg="red",
            bg="silver",
            command=lambda: global_class_file_.left(game),
        )
        C = tk.Button(
            text="Отменить",
            bd=5,
            ...
            fg="red",
            bg="silver",
            command=lambda: global_class_file_.cancel_turn(game),
        )
        F = tk.Button(
            text="Вперёд",
            bd=5,
            ...
            fg="red",
            bg="silver",
            command=lambda: global_class_file_.forward_turn(game),
        )
        game.HELP_BUTTON = tk.Button(
            text="Забыли персонажа?",
            bd=5,

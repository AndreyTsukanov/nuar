def choose():
    ...
    def choose_size(x):
        ...
        begin_topic = tk.Label(
            text="Выберите число игроков", font=("Arial, 60"), bg="#DAA520"
        )
        ...
        for i in range(2, 7):
            but = tk.Button(
                ...
                font=("Arial, 15"),
                bd=10,
                command=lambda x=i: choose_players(x),
            )
    ...
    for i in range(3, 7):
        but = tk.Button(
            ...
            bg="#DAA520",
            bd=10,
            command=lambda x=i: choose_size(x),
        )
        but.place(x=-180 + 200 * i, y=400)
        list_size.append(but)
    begin_topic = tk.Label(
        text="Выберите размер поля", font=("Arial, 60"), bg="#DAA520"
    )

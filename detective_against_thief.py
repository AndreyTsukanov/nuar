def mod_3():
    ...
    class DVST:
        def __init__(self):
            ...
            self.Table = [
                [["", 0, "#ebc923"] for j in range(size)] for k in range(size)
            ]
            self.NAMES = [
                "Ксавье",
                "Ирма",
        ...
        for i in range(len(game.Table)):
            ...
            for j in range(len(game.Table[0])):
                if game.Table[i][j][1] == 2:
                    but = tk.Button(
                        text=game.Table[i][j][0],
                        bd=15,
                        font=("TimesNewRoman", 13, "underline"),
                        fg="blue",
                        bg=game.Table[i][j][2],
                        width=10,
                        height=4,
    ...
    def help_with_player():
        game.turn.destroy()
        game.turn = tk.Label(
            text="Ход " + game.dict_name[game.player_number],
            font=("Arial, 30"),
            bg=game.bg_,
        )
        if game.player_number == 1:
            ...
            game.change_ = tk.Button(
                ...
                fg="red",
                bg="silver",
                command=lambda: CHANGE_THIEF(),
            )
            game.steal = tk.Button(
                text="Украсть золото",
                bd=5,
                font=("Arial", 10),
                width=20,
            ...
            game.change_officer = tk.Button(
                text="Сменить офицера",
                bd=5,
                font=("Arial", 10),
                width=20,
                height=2,
    ...
    def cancel_turn():
        ...
        game.hint_topic = tk.Label(text="Подсказка:", font=("Arial, 30"), bg=game.bg_)
        game.hint_label = tk.Label(
            text="Вы вернулись в начало игры!",
            font=("Arial, 15"),
            bg=game.bg_,
            fg="black",
        )
    ...
    def forward_turn():
        ...
        game.hint_topic = tk.Label(text="Подсказка:", font=("Arial, 30"), bg=game.bg_)
        game.hint_label = tk.Label(
            text="Вы вернулись в конец игры!",
            font=("Arial, 15"),
            bg=game.bg_,
            fg="black",
        )
    ...
    def CHANGE_THIEF():
        help_clear()
        game.hint_label = tk.Label(
            text="ВОР! Это твои тайные личности.\nВыбери личность ТАК, ЧТОБЫ\nЭТОГО НИКТО НЕ ВИДЕЛ!",
            font=("Arial, 20"),
            bg=game.bg_,
        )
    ...
    def CHANGE_OFFICER():
        ...
        if len(game.Clicked_Buttons) != 1:
            ...
            if game.Table[i][j][1] != 2:
                ...
                game.hint_label = tk.Label(
                    text="Свободных офицеров не осталось!",
                    font=("Arial, 16"),
                    bg=game.bg_,
                    fg="black",
                )
                ...
                for i in range(len(game.Table)):
                    for j in range(len(game.Table[0])):
                        if game.Table[i][j][0] == name:
                            game.Table[i][j][1] = 2
                            game.detective_cords.append([i, j])
                game.hint_label = game.hint_label = tk.Label(
                    text=f"СЫЩИК! Ваш новый офицер - {name}",
                    font=("Arial, 16"),
                    bg=game.bg_,
                    fg="black",
                )
    ...
    def QUESTION():
        ...
        if len(game.Clicked_Buttons) != 1:
            ...
            if (
                (abs(x_1 - i) > 1 or abs(y_1 - j) > 1)
                and (abs(x_2 - i) > 1 or abs(y_2 - j) > 1)
                and (abs(x_3 - i) > 1 or abs(y_3 - j) > 1)
            ):
                game.hint_label = tk.Label(
                    text="Ваши офицеры находятся не рядом\n с выбранным Вами персонажем.",
                    font=("Arial, 16"),
                    bg=game.bg_,
                    fg="black",
                )
                ...
                game.hint_label = tk.Label(
                    text=f"{game.Table[i][j][0].upper()} не является \nтекущей личностью вора.",
                    font=("Arial, 20"),
                    bg=game.bg_,
                    fg="black",
                )
        ...
        if len(game.Clicked_Buttons) != 1:
            ...
            if game.Table[i][j][2] != "#ebc923":
                ...
                if abs(x - i) > 1 or abs(y - j) > 1:
                    game.hint_label = tk.Label(
                        text="Ваша тайная личность находится\n не рядом с выбранной золотой клеткой",
                        font=("Arial, 16"),
                        bg=game.bg_,
                        fg="black",
                    )
    ...
    game.begin_button = tk.Button(
        ...
        width=20,
        height=2,
        command=lambda: help_for_begin_1(),
    )
            ...
            for i in range(len(game.Table)):
                for j in range(len(game.Table[0])):
                    if game.Table[i][j][0] == name:
                        game.detective_cords.append([i, j])
        game.hint_label = tk.Label(
            text=f"СЫЩИК! Ваши офицеры - это \n {game.list_[0]}, {game.list_[1]} и {game.list_[2]}!",
            font=("Arial, 20"),
            bg=game.bg_,
        )
    ...
    def help_for_begin_2():
        ...
        for j in range(3):
            but = tk.Button(
                ...
                width=10,
                height=4,
                command=lambda x=j: (change_thief(game.list_[x]), help_for_begin_3()),
            )
            game.list_buttons.append(but)
    ...
    def function_OK():
        ...
        game.begin_button = tk.Button(
            ...
            width=30,
            height=2,
            command=lambda: help_for_begin_2(),
        )

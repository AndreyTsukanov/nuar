...
def choose_size(x):
    ...
    for i in range(2, 7):
        but = tk.Button(text=str(i), width=10,height=4,bg="#DAA520",font=('Arial, 15'),command=lambda x=i: choose_players(x))
...
for i in range(3, 7):
    but = tk.Button(text=str(i), width=10,height=4,font=('Arial, 15'),bg = "#DAA520",command=lambda x=i: choose_size(x))

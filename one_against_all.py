def mod_1():
                ...
                if (
                    ...
                    if game.killed_person != 0:
                        if game.people_alive >= players:
                            ...
                            game.dead_topic.place(x=30, y=500)
                            game.dead_label = tk.Label(
                                ...
                                + ". \nИгрок номер "
                                + str(game.killed_person)
                                + ", укажите свое \nместо возрождения так, чтобы это никто \nне видел, и нажмите кнопку 'ОК'.",
                                font=("Arial, 15"),
                                bg=game.bg_,
                                fg="black",
                            )
                            game.dead_label.place(x=40, y=560)
                            ...
                            game.dead_button.place(x=180, y=690)
                        else:
                            for x in range(len(game.Table)):
                                for y in range(len(game.Table[0])):
                                    if game.Table[x][y][2] != "black":
                            ...
                            game.dead_label = tk.Label(
                                ...
                                + ". \nИгрок номер "
                                + str(game.killed_person)
                                + ", Вы не можете больше\n возродиться, так как свободных живых\n клеток не осталось. Нажмите кнопку 'ОК'.",
                                font=("Arial, 15"),
                                bg=game.bg_,
                                fg="black",
                            )
    ...
    def question():
        ...
        game.hint_topic = tk.Label(text="Подсказка:", font=("Arial, 30"), bg=game.bg_)
        game.hint_label = tk.Label(
            text="Укажите ячейку персонажа, находящуюся\n рядом с Вашим пресонажем,\n чтобы опросить его...",
            font=("Arial, 16"),
            bg=game.bg_,
            fg="black",
        )
    ...
    def help_but_click(x, y):
        if [x, y] not in game.Clicked_Buttons:
            if game.killed_person == 0:
                but = tk.Button(
                    text=global_class_file_.help_to_show(game.Table[x][y][0]),
                    bd=5,
                    width=10,
                    height=4,
                    font=("Arial", 13),
                    bg="gray",
                    fg="black",
                    command=lambda i=x, j=y: help_but_click(i, j),
                )
            else:
                but = tk.Button(
                    text="",
        ...
        for i in range(len(game.Table)):
            ...
            for j in range(len(game.Table[0])):
                but = tk.Button(
                    ...
                    width=10,
                    height=4,
                    command=lambda x=i, y=j: help_but_click(x, y),
                )
    ...
    def help_for_begin():
        if game.num <= players:
            if (
                len(game.Clicked_Buttons) == 1
                and game.Clicked_Buttons[0] not in game.player_cords
            ):
                game.hint_label.destroy()
                game.player_cords.append(game.Clicked_Buttons[0])
                game.Table[game.Clicked_Buttons[0][0]][game.Clicked_Buttons[0][1]][
                    1
                ] = game.num
                game.num += 1
                if game.num <= players:
                    ...
                    game.hint_label = tk.Label(
                        text="Да начнется игра!", font=("Arial, 40"), bg=game.bg_
                    )
                    game.hint_label.place(x=10, y=350)
            show_empty_table()
        else:
